...
void Client::loadParticles(const std::string &filename) {
    XmlReader xr(filename);
    if (!xr)
        return;
...
void Client::loadSounds(const std::string &filename) {
    XmlReader xr(filename);
    if (!xr)
        return;
...
void Client::loadProjectiles(const std::string &filename) {
    XmlReader xr(filename);
    if (!xr)
        return;
...
void Client::loadSpells(const std::string &filename) {
    XmlReader xr(filename);
    if (!xr)
        return;
...
void Client::loadBuffs(const std::string &filename) {
    XmlReader xr(filename);
    if (!xr)
        return;
...
void Client::loadObjectTypes(const std::string &filename) {
    XmlReader xr(filename);
    if (!xr)
        return;
...
void Client::loadItems(const std::string &filename) {
    XmlReader xr(filename);
    if (!xr)
        return;
...
void Client::loadClasses(const std::string &filename) {
    XmlReader xr(filename);
    if (!xr)
        return;
...
void Client::loadRecipes(const std::string &filename) {
    XmlReader xr(filename);
    if (!xr)
        return;
...
void Client::loadNPCTypes(const std::string &filename) {
    XmlReader xr(filename);
    if (!xr)
        return;
...
void Client::loadMap(const std::string &filename) {
    XmlReader xr(filename);
    if (!xr) {
        return;
    }
    ...
    for (auto row : xr.getChildren("row")) {
        ...
        if (!rowNumberSpecified) {
            showErrorMessage("Map row is missing a row number.", Color::FAILURE);
            return;
        }
        if (y >= _mapY) {
            showErrorMessage("Map row number "s + toString(y) + " exceeds y dimension of " +
                toString(_mapY), Color::FAILURE);
            return;
        }
        std::string rowTerrain;
        if (!xr.findAttr(row, "terrain", rowTerrain)) {
            showErrorMessage("Map row is missing terrain information.", Color::FAILURE);
            return;
        }
        for (size_t x = 0; x != rowTerrain.size(); ++x) {
            if (x > _mapX) {
                showErrorMessage("Row length of "s + toString(x) + " exceeds x dimension of " +
                    toString(_mapX), Color::FAILURE);
                return;
            }
