...
void CDataLoader::loadSpells(XmlReader &xr) {
    ...
    for (auto elem : xr.getChildren("spell")) {
        ...
        if (xr.findAttr(elem, "icon", icon))
            newSpell->icon(_client._icons[icon]);
        ...
        if (aesthetics) {
            ...
            if (xr.findAttr(aesthetics, "projectile", profileName)) {
                ...
                if (it != _client._projectileTypes.end())
...
void CDataLoader::loadObjectTypes(XmlReader &xr) {
    ...
    for (auto elem : xr.getChildren("objectType")) {
        ...
        if (xr.findAttr(elem, "gatherParticles", s)) cot->gatherParticles(_client.findParticleProfile(s));
        if (xr.findAttr(elem, "damageParticles", s)) cot->damageParticles(_client.findParticleProfile(s));
        ...
        if (canConstruct) {
            ...
            if (!hasLocks)
                _client._knownConstructions.insert(id);
        }
        ...
        if (strength) {
            if (xr.findAttr(strength, "item", s) &&
                ...
                _client.showErrorMessage("Transformation specified without target id; skipping.",
                    Color::FAILURE);
        }
...
void CDataLoader::loadRecipes(XmlReader &xr) {
    ...
    for (auto elem : xr.getChildren("recipe")) {
        ...
        if (it == _client._items.end()) {
            _client.showErrorMessage("Skipping recipe with invalid product "s + s, Color::FAILURE);
            continue;
        }
        ...
        for (auto child : xr.getChildren("material", elem)) {
            ...
            if (xr.findAttr(child, "id", s)) {
                auto it = _client._items.find(s);
                if (it == _client._items.end()) {
                    _client.showErrorMessage("Skipping invalid recipe material "s + s, Color::FAILURE);
                    continue;
                }
...
void CDataLoader::loadNPCTypes(XmlReader &xr) {
    ...
    for (auto elem : xr.getChildren("npcType")) {
        ...
        for (auto gearElem : xr.getChildren("gear", humanoid)) {
            ...
            if (it == _client._items.end()) {
                _client.showErrorMessage("Skipping invalid NPC gear "s + id, Color::FAILURE);
                continue;
            }
...
void CDataLoader::loadMap(XmlReader &xr) {
    ...
    if (elem == nullptr || !xr.findAttr(elem, "x", _client._mapX) || !xr.findAttr(elem, "y", _client._mapY)) {
        _client.showErrorMessage("Map size missing or incomplete.", Color::FAILURE);
        return;
    }
    ...
    for (auto row : xr.getChildren("row")) {
        ...
        if (!rowNumberSpecified) {
            _client.showErrorMessage("Map row is missing a row number.", Color::FAILURE);
            return;
        }
        if (y >= _client._mapY) {
            _client.showErrorMessage("Map row number "s + toString(y) + " exceeds y dimension of " +
                toString(_client._mapY), Color::FAILURE);
            return;
        }
        std::string rowTerrain;
        if (!xr.findAttr(row, "terrain", rowTerrain)) {
            _client.showErrorMessage("Map row is missing terrain information.", Color::FAILURE);
            return;
        }
        for (size_t x = 0; x != rowTerrain.size(); ++x) {
            if (x > _client._mapX) {
                _client.showErrorMessage("Row length of "s + toString(x) + " exceeds x dimension of " +
                    toString(_client._mapX), Color::FAILURE);
                return;
            }
