...
void HelpEntry::addParagraph(size_t order, const std::string &heading,
 ...
 public:
  void addWordsFromString(const std::string &paragraph,
    ...
    while (iss) {
      ...
      label->matchW();
      if (color != Color::FONT) label->setColor(color);
      label->refresh();
...
void HelpEntry::draw(List *page) const {
  ...
  for (const auto &paragraph : _paragraphs) {
    ...
    words.addNewLine();
    if (!isFirstParagraphInBlock && !pageIsEmpty) words.addTab();
