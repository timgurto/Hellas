...
const std::vector<XP> User::XP_PER_LEVEL{
    ...
    // [59] = XP required to get to lvl 60
        0,    600,   1200,   1700,   2300,
     2800,   3300,   3800,   4300,   4800,
     5200,   5700,   6100,   6500,   6900,
     7300,   7700,   8100,   8400,   8800,
     9100,   9500,   9800,  10100,  10400,
    10700,  11000,  11300,  11600,  11900,
    12100,  12400,  12700,  12900,  13200,
    13400,  13600,  13900,  14100,  14300,
    14500,  14800,  15000,  15200,  15400,
    15600,  15800,  16000,  16100,  16300,
    16500,  16700,  16900,  17000,  17200,
    17400,  17500,  17700,  17800,  18000
};
...
void removeItemsFrom(ItemSet &remaining, ServerItem::vect_t &container, std::set<size_t> &slotsChanged) {
    slotsChanged = {};
...
void User::removeItems(const std::string & tag, size_t quantity) {
    ...
    slotsChanged = {};
    for (size_t i = 0; i != User::GEAR_SLOTS; ++i) {
        std::pair<const ServerItem *, size_t> &gearSlot = _gear[i];
        if (gearSlot.first->isTag(tag)) {
            size_t itemsToRemove = min(gearSlot.second, remaining);
            remaining -= itemsToRemove;

            _gear[i].second -= itemsToRemove;
            if (_gear[i].second == 0)
