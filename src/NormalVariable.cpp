...
NormalVariable::NormalVariable(double mean, double sd)
    : _zeroSD(false), _mean(mean), _sd(sd), _dist(nullptr) {
  if (sd == 0)
    _zeroSD = true;
  else
...
NormalVariable::NormalVariable(const NormalVariable &rhs)
    : _zeroSD(rhs._zeroSD), _mean(rhs._mean), _sd(rhs._sd), _dist(nullptr) {
  if (!_zeroSD) _dist = new std::normal_distribution<double>(_mean, _sd);
}

NormalVariable::~NormalVariable() {
  if (_dist != nullptr) delete _dist;
}
