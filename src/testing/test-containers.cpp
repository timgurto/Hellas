...
TEST_CASE_METHOD(ServerAndClientWithData,
  ...
  CHECK_FALSE(client->getFirstObject().container().empty());
}

TEST_CASE_METHOD(ServerAndClientWithData,
                 "Merchant can use same slot for ware and price",
                 "[containers][merchant]") {
  GIVEN("a merchant object with one inventory slot, containing the ware") {
    ...
    server->addObject("diamondStore", {10, 15});
    auto &store = server->getFirstObject();
    const auto *diamond = (*server)->findItem("diamond");
    ...
    WHEN("a user with the price tries to buy the ware") {
      ...
      client->sendMessage(CL_TRADE, makeArgs(store.serial(), 0));
...
TEST_CASE_METHOD(ServerAndClientWithData,
  ...
  GIVEN("rainclouds disappear when empty; rainwater tanks don't") {
    ...
    AND_GIVEN("a raincloud with rain") {
      ...
      WHEN("the user moves the rain into his inventory") {
      ...
      WHEN("the user 'takes' the rain)") {
        client->sendMessage(CL_TAKE_ITEM, makeArgs(raincloud.serial(), 0));
    ...
    AND_GIVEN("a rainwater tank with rain") {
      ...
      WHEN("the rain is removed") {
        auto itemsToRemove = ItemSet{};
        itemsToRemove.add(rain);
        rainwaterTank.container().removeItems(itemsToRemove);
